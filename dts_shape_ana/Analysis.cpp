...
{
    Nfunction f;
    m_TRJ = pState->GetTrajectory(); /// read all the trajectory in the folder
    ...
    std::ofstream dircur;
    dircur.open("c1c2area.xvg");
    dircur<<"c1   c2    area \n";
    
    if(istep>m_TRJ.size()-1)
    std::cout<<" error-----> ini step "<<istep<<" is larger then the number of the frames \n";
    for (std::vector<MeshBluePrint>::iterator it = m_TRJ.begin() ; it != m_TRJ.end(); ++it)  /// loop over all the frames 
    {
       if(i>=istep && i<=fstep)
       {
            MESH mesh;   // you need this
            mesh.GenerateMesh(*it, k, kg);   // you need this

            UpdateEnergy(mesh);    // you need this update geometry and energy
        ...
        WritevtuFiles vtu;
        //vtu.WritevtuNochange();  this should be completed 
